...
public class MyBooksAdapter extends ArrayAdapter<Ebook> {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        if (listItem == null) {
            ...
            mBookPages.setText(String.format("n° pages : %s", current.getPages() == 0 ? "-" : String.valueOf(current.getPages())));

            MaterialButton mReadBtn = listItem.findViewById(R.id.read_btn);
            mReadBtn.setOnClickListener(view -> Toast.makeText(getContext(), "button clicked !", Toast.LENGTH_SHORT).show());
        }
