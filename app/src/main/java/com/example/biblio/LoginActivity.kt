...
class LoginActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.loginBtn.setOnClickListener { _: View? ->
            ...
            if (validateEmail(email)) executeLogin(UserBuilder().setEmail(email).setPassword(password).build(), true)
        }
    ...
    private fun executeLogin(user: User, showProgress: Boolean) {
        val dialog = initProgress(showProgress)
        Thread(Runnable {
            val successful = user.login()
            if (showProgress) runOnUiThread { dialog?.dismiss() }
            if (successful) {
